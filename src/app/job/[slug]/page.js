        ...
        <div className="container">
          <div className="flex justify-between gap-12 items-center border-b pb-4 border-greyPrimary">
            <div className="flex flex-col gap-8">
              <button className="py-2 px-4 border border-bluePrimary rounded-2xl text-sm w-[10rem] flex justify-center items-center">
                {job.category}
              </button>

              <h4 className="custom-h2 font-medium font-mont text-bluePrimary cursor-pointer">
                {job.title}
              </h4>
            </div>
            ...
            <div className="apply-button">
              <button 
                className="p-4 text-white bg-bluePrimary text-sm w-[10rem] flex justify-center items-center" 
          ...
          <div className="my-12 flex flex-col gap-6">
            <h4 className="custom-h4 font-medium font-mont text-bluePrimary cursor-pointer">
              About {job.aboutCompany.name}
            </h4>

            <h6 className="custom-h6 font-regular font-mont text-bluePrimary cursor-pointer">
            ...
            <div className="services mt-6">
              {job.aboutCompany.services.map((service, index) => (
                <div key={index} className="service-item my-12">
                  <h5 className="custom-h5 font-medium font-mont text-bluePrimary text-center cursor-pointer">
                    {service.name}
                  </h5>
                  <h6 className="custom-h6 font-regular font-mont text-bluePrimary cursor-pointer pt-6">
                    {service.description}
                  </h6>
                </div>
              ))}
          ...
          <div className="my-12 flex flex-col">
            <h4 className="custom-h4 font-medium font-mont text-bluePrimary cursor-pointer text-center">
              Overview:
            </h4>
          ...
          <div className="my-12 flex flex-col">
            <h4 className="custom-h4 font-medium font-mont text-bluePrimary text-center">
              ...
              {Object.keys(job.responsibilities).map((key, index) => {
                ...
                return (
                  <div key={index} className="responsibility-item my-12">
                    <h5 className="custom-h5 font-medium font-mont text-bluePrimary cursor-pointer">
                    ...
                    <ul className="custom-list mt-4">
                      {responsibility.points.map((point, idx) => (
                        <li
                          key={idx}
                          className="flex items-center custom-h6 custom-list-item font-regular font-mont text-bluePrimary my-6 pl-12"
                        >
                          {point}
                        </li>
                      ))}
